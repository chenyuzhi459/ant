...
//    private static final Logger log = new Logger(JsonConfigurator.class);
    private static final Logger log = LogManager.getLogger(JsonConfigurator.class);
    ...
    public <T> T configurate(Properties props, String propertyPrefix, Class<T> clazz) throws ProvisionException {
        try {
            ...
            for (String prop : props.stringPropertyNames()) {
                if (prop.startsWith(propertyBase)) {
                    ...
                    try {
                        ...
                        String errMsg = String.format("Unable to parse [%s]=[%s] as a json object, using as is.", prop, propValue);
                        log.error(errMsg, e);
                        value = propValue;
                    }
            ...
            log.info(String.format("Loaded class[%s] from props[%s] as [%s]", clazz, propertyBase, config));
            return config;
        } catch (Exception e) {
            String errMsg = String.format("Load class[%s] failed", clazz);
            log.error(errMsg, e);
            throw new RE(e, "Load class[%s] failed", clazz);
        }
    }
